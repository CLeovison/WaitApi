...
public class UserRepositories(IDbConnectionFactory connectionString) : IUserRepositories
{
    ...
    public async Task<bool> RegisterUserAsync(UserDto user)
    {
        ...
        var result = await connection.ExecuteAsync(@"INSERT INTO waitdb( username,password,firstname,lastname,birthday, email) VALUES ( @username, @password, @firstname, @lastname, @birthday, @email)", user);
        return result > 0;
    }
    public async Task<IEnumerable<UserDto>> ExistingUserAsync(string email, string username){
        using var connection = await connectionString.CreateConnectionAsync();
        var result =  await connection.QueryAsync<UserDto>(@"SELECT username,email FROM waitdb WHERE username = @username, email = @email", );
        return result;
    }
    ...
    {
        using var connection = await connectionString.CreateConnectionAsync();
        return await connection.QuerySingleOrDefaultAsync(@"SELECT * FROM waitdb WHERE username = @username", username);
    }
